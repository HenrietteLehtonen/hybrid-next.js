...
const MediaForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const formData = new FormData(e.currentTarget); // current target viittaa lomakkeeseen

      // send the form data to Next.js API endpoint /api/media using fetchData function
      const options = {
        method: "POST",
        body: formData, // formdatan kanssa ei headereita
      };
      ...
      // lisätään sovelluskohtainen tag
      const apptagData = {
        tag_name: "omaAppitagi",
        media_id: uploadResult.media.media_id,
      };
      const apptagOptions = {
        method: "POST",
        ...
        body: JSON.stringify(apptagData),
      };
      ...
      if (!uploadResult) {
        throw new CustomError("Error while uploading media", 500);
      }
      router.push("/");
    } catch (error) {
      console.error(error);
